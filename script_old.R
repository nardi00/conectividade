...
NOMES   <- c("ITUB4", "BBDC4", "BBAS3", "SANB11",
             "BPAC11", "BRSR6", "ABCB4", "BPAN4")   # nomes das abas no Excel (N = 8)
...
EVENTOS <- data.frame(
  data  = as.Date(c("2020-03-11", "2022-01-01",
                    "2023-01-12", "2024-10-01")),
  label = c("COVID-19", "Aperto Selic",
            "Americanas", "Crise Fiscal 2024"),
  stringsAsFactors = FALSE
)

# ==============================================================================
# SEÇÃO 1 — LEITURA DOS DADOS (EXCEL — COMDINHEIRO)
# ==============================================================================
...
for (nome in NOMES) {
  ...
  if (nrow(raw) == 0) {
    stop("Nenhuma observação encontrada para ", nome,
         " no período ", DATA_INICIO, " a ", DATA_FIM, ".")
  }
  
  # As cotações do ComDinheiro (flag_ajusta = 1, ver aba LEIA-ME) já vêm
  # ajustadas por proventos E cisões, então as colunas *_Aj são usadas
  # diretamente — não é mais necessário recalcular o fator de ajuste phi
  # (etapa que existia apenas porque o Yahoo Finance não ajusta cisões).
  ohlc_adj <- xts(
    data.frame(
      ...
      Volume_BRL = raw$Volume_MM_RS * 1e6   # Volume_MM_RS já é R$ (milhões) -> R$
    ),
    order.by = raw$Data
  )
...
# O arquivo do ComDinheiro não traz volume em quantidade de ações, apenas o
# volume financeiro (Volume_MM_RS). Por isso o filtro por quantidade mínima
# de ações (VOLUME_MIN_QTD) do script original foi removido, mantendo-se
# apenas o piso de volume financeiro.
...
# Não é mais necessário truncar BPAC11 a partir de 2017-08-01: como o
# período de análise agora começa em 2019-01-01, todos os bancos (inclusive
# BPAC11, que só existe a partir de 16/02/2017) já têm série completa.
...
for (nome in NOMES) {
  ...
  # Verificar NAs antes (não são mais interpolados aqui — ver nota abaixo)
  n_nas <- sum(is.na(ohlc_mat))
  if (n_nas > 0) cat(" ", nome, ":", n_nas, "NAs no OHLC (mantidos até a Seção 7)\n")
  
  # NOTA METODOLÓGICA: por ora, os NAs do OHLC (filtro de liquidez, sanity
  # checks) NÃO são interpolados antes do Yang-Zhang — seguem como NA para
  # dentro de TTR::volatility(), que por sua vez propaga NA para toda janela
  # rolante de N_JANELA_YZ dias que contenha algum NA. A imputação de fato
  # (filtro de Kalman) só ocorre na Seção 7, sobre o painel de log-volatilidade
  # já alinhado — conforme descrito na Seção 6.1.3 do texto (etapa 6).
  #
  # Isso é diferente do que o script fazia antes (interpolação linear +
  # LOCF direto no OHLC, ANTES do Yang-Zhang) — mantido abaixo comentado
  # para comparação posterior dos resultados entre as duas abordagens.
  ohlc_imp <- ohlc_mat
  
  # --- INTERPOLAÇÃO NO OHLC (desativada — comparar depois) -------------------
  # ohlc_imp <- ohlc_mat
  ...
  if (n_nas_vol > 0) {
    cat("   ", nome, "→", n_nas_vol, "NAs na volatilidade estimada",
        "(aquecimento da janela + propagação de NAs do OHLC não interpolado)\n")
  }
}

cat("  OK — volatilidade Yang-Zhang estimada para", length(vol_yz), "séries.\n")
cat("  (NAs propagados sem interpolação prévia do OHLC — a imputação por\n",
    "  Kalman na Seção 7 é quem trata isso por enquanto.)\n")
...
for (nome in NOMES) {
  ...
  ers_res <- tryCatch(
    ur.ers(lv_limpo, type = "DF-GLS", model = "constant", lag.max = 4),
  ...
  pp_res <- tryCatch(
    ur.pp(lv_limpo, type = "Z-tau", model = "constant", lags = "short"),
    error = function(e) NULL
  )
  ...
  # ── Teste Zivot-Andrews (raiz unitária com quebra estrutural) ─────────────
  za_res <- tryCatch(
    ur.za(lv_limpo, model = "both", lag = 4),
    error = function(e) NULL
  )
  za_stat    <- if (!is.null(za_res)) za_res@teststat else NA
  za_cv5     <- if (!is.null(za_res)) za_res@cval[2] else NA   # cval = c(1%, 5%, 10%), sem nomes
  ...
  resultados_testes <- rbind(resultados_testes, data.frame(
    ...
    DFGLS_stat       = round(ers_stat, 3),
    DFGLS_cv5pct     = round(ers_cv5, 3),
    DFGLS_rejeita_H0 = ers_rejeita,
    PP_stat          = round(pp_stat, 3),
    PP_cv5pct        = round(pp_cv5, 3),
    PP_rejeita_H0    = pp_rejeita,
    ZA_stat          = round(za_stat, 3),
    ZA_cv5pct        = round(za_cv5, 3),
    ZA_rejeita_H0    = za_rejeita,
...
if (length(falhas_dfgls) > 0)
  warning("DF-GLS NÃO rejeitou raiz unitária em: ", paste(falhas_dfgls, collapse=", "))
if (length(falhas_pp) > 0)
  warning("Phillips-Perron NÃO rejeitou raiz unitária em: ", paste(falhas_pp, collapse=", "))
if (length(falhas_za) > 0)
  warning("Zivot-Andrews NÃO rejeitou raiz unitária (mesmo com quebra) em: ", paste(falhas_za, collapse=", "))
...
cat("\n[10/10] Análise dinâmica — janela rolante (W =", N_JANELA_W,
    ", H =", H_HORIZONTE, ")...\n")
cat("  (Isso pode levar alguns minutos)\n")

# Não é mais necessário truncar o painel para o início comum de BPAC11
# (2017-08-01): o período de análise já começa em 2019-01-01, quando todos
# os 8 bancos têm dados.
cat("Painel:", nrow(Y), "observações | de:",
...
p_tci <- ggplot(tci_df, aes(x = data, y = TCI)) +
  ...
  labs(
    title    = "Total Connectedness Index — Sistema Bancário Brasileiro (2019–2025)",
